...
echo "🔍 Searching for .IN.NetworkFile.graphML files in $DIR_4_MINED_NETWORKS_NOFI_GRAPHML"
files=($(find $DIR_4_MINED_NETWORKS_NOFI_GRAPHML -type f -name "*.IN.NetworkFile.graphML" 2>/dev/null | sort))

# Check if any files were found
if [[ ${#files[@]} -eq 0 ]]; then
    echo "❌ No .IN.NetworkFile.graphML files found in current directory or subdirectories."
    exit 1
fi

echo "✅ Found ${#files[@]} .IN.NetworkFile.graphML file(s):"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
...
if [[ ${#selected_files[@]} -gt 0 ]]; then
    ...
    if [[ $confirm =~ ^[Yy]$ ]]; then
        ...
        for file in "${selected_files[@]}"; do
            ...
            base_name="${file%.IN.NetworkFile.graphML}"
            output_file="${base_name}.out.graphML"

            echo "   → Output will be: $output_file"

            # Call your Python script here
            # Replace with your actual Python command
            # python3 your_deanonymize_script.py "$file" "$output_file"

            # For demonstration, just showing the command
            echo "   → Command: python3 deanonymize_github_user.py --config=$GIT_HUB_AUTH_CONFIG \"$file\" \"$output_file\""



            if $DEANONYMIZE_GRAPHML_SCRIPT  --config=$GIT_HUB_AUTH_CONFIG "$file" "$output_file"; then
